...
begin
  ...
  ## Write out all of the efficient frontier files (LONG!) ##

  csv_location = "#{Rails.root}/db/data/efficient_frontiers"
  FileUtils.mkdir_p csv_location
  FileUtils.rm_rf(Dir.glob("#{csv_location}/*")) # Clear it

  beginning_time = Time.now

  3.upto(tickers.count) do |n|

    puts "..... ON #{n} SECURITIES ......"

    combos = tickers.combination(n).to_a

    combos.each_with_index do |combo, index|

      if index%100 == 0
        pct_complete = (index.to_f/combos.count) * 100
        puts "#{pct_complete}% complete - #{n} assets."
      end
      ...
      # Use the 'a' flag because we DO want to append to the end of a file (multiple combinations of three assets)
      CSV.open("#{csv_location}/#{portfolio_weights.first.length}_asset_eff_front.csv", "a") do |csv|
        # Header row
        array = portfolio_weights.first.keys
        array.map! {|i| i.to_s}
        array.unshift "Portfolio ID"
        csv << array

        # Push all portfolio allocation combinations to the CSV
